...
function specs_show_tags(){
	...
	foreach($tag_cache as $value){
		$value['usenum'] = $value['usenum'] <=0 ? '1' : $value['usenum'];
		$vs = array('lfterm_url'=>$value['tagurl'],'lfname'=>$value['tagname'],'lfnum'=>$value['usenum']);
	...
	ksort($lfcategories);
	for($i=65;$i<=90;$i++){
		$tagi = $lfcategories[chr($i)];
		if(is_array($tagi)){
			echo "<div class=\"wrap-tag-list\" id='".chr($i)."'><span class=\"character\">".chr($i)."</span><div class=\"clearfix\"></div>";
			foreach($tagi as $tag){
				echo "<div class=\"tag-item\"><a href=\"".Url::tag($tag['lfterm_url'])."\" class=\"q-tag\">{$tag['lfname']}</a> x {$tag['lfnum']}</div>";
			}
			echo "</div>";
		}
	}
